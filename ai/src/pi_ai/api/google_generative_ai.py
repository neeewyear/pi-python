...
def format_google_error(error: object) -> str:
    """格式化 Google 错误。"""
    return format_provider_error(normalize_provider_error(error), "Google API error")


def _get_base_url(model: Any) -> str:
    """获取模型的基础 URL。"""
    base_url = getattr(model, "base_url", None) or ""
    return base_url.strip("/") if base_url else _GEMINI_API_BASE
...
def _get_api(
    ...
    return get_client_api_key(provider, api_key, options.headers if options else None)


# ---------------------------------------------------------------------------
# create_client - 创建 HTTP 客户端配置
# ---------------------------------------------------------------------------
...
def _get_http_headers(
    ...
    # 合并模型级 headers
    model_headers = getattr(model, "headers", None)
    if model_headers and isinstance(model_headers, dict):
...
def _is_gemma_4_model(model_id: str) -> bool:
    """检查是否为 Gemma 4 模型。"""
    import re

    return bool(re.search(r"gemma-?4", model_id.lower()))
...
async def _make_google_request(
    ...
    """发起 Google Gemini API 流式请求并返回解析后的 JSON 块。"""
    url = f"{base_url}/v1beta/models/{model_id}:streamGenerateContent"
...
def stream(
    ...
    async def _run() -> None:
        ...
        try:
            ...
            # 流式处理
            # current_block tracking: ("text", index) or ("thinking", index)
            ...
            output.error_message = format_google_error(error)
            event_stream.push(
...
def stream_simple(
    ...
    if (
        _is_gemini_3_pro_model(model_id)
        or _is_gemini_3_flash_model(model_id)
        or _is_gemma_4_model(model_id)
    ):
        return stream(
            model,
            context,
