...
def _grapheme_width(segment: str) -> int:
    ...
    if cat in ("Mn", "Me", "Cf", "Cc", "Cs", "Co", "Cn"):
        ...
        if all(
            unicodedata.category(c) in ("Mn", "Me", "Cf", "Cc", "Cs") for c in segment
        ):
            return 0
...
try:
    from grapheme import (  # type: ignore[import-not-found]
        graphemes as _grapheme_lib_split,  # type: ignore[import-not-found]
    )
...
def _segment_graphemes(text: str) -> list[str]:
    ...
    while i < n:
        ...
        if (
            _is_regional_indicator(cp)
            and i + 1 < n
            and _is_regional_indicator(ord(text[i + 1]))
        ):
            cluster = text[i : i + 2]
            i += 2
...
def visible_width(s: str) -> int:
    ...
    if len(_width_cache) >= _WIDTH_CACHE_SIZE:
        oldest = (
            _width_cache_order.pop(0)
            if _width_cache_order
            else next(iter(_width_cache))
        )
...
def _wrap_single_line(
    line: str, width: int, tracker: AnsiCodeTracker | None = None
) -> list[str]:
    if not line:
        return [""]
...
def apply_background_to_line(
    ...
    return bg_fn(line + " " * padding)  # type: ignore[no-any-return]
