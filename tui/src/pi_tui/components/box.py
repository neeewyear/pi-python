...
class Box:
    ...
    def __init__(
        ...
        self._bg_fn = bg_fn
        self._cache: dict | None = None  # type: ignore[type-arg]
    ...
    def render(self, width: int) -> list[str]:
        ...
        if (
            c is not None
            and c["width"] == width
            and c["bg_sample"] == bg_sample
            and len(c["child_lines"]) == len(child_lines)
            and all(
                c["child_lines"][i] == child_lines[i] for i in range(len(child_lines))
            )
        ):
            return c["lines"]  # type: ignore[no-any-return]
