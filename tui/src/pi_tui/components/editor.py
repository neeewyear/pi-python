...
@dataclass
class EditorTheme:
    border_color: Callable[[str], str] = lambda x: x
...
def word_wrap_line(line: str, max_width: int) -> list[TextChunk]:
    ...
    for i, g in enumerate(graphemes):
        ...
        if current_width + g_width > max_width:
            if wrap_opp_idx >= 0:
                chunks.append(
                    TextChunk(line[chunk_start:wrap_opp_idx], chunk_start, wrap_opp_idx)
                )
                chunk_start = wrap_opp_idx
                current_width -= wrap_opp_width
            elif chunk_start < char_index:
                chunks.append(
                    TextChunk(line[chunk_start:char_index], chunk_start, char_index)
                )
...
class Editor:
    ...
    def set_autocomplete_max_visible(self, max_visible: int) -> None:
        ...
        if self._autocomplete_max_visible != new_max:
            ...
            if self._autocomplete_list:
                ...
                self._autocomplete_list.on_select = self._apply_autocomplete
                self._autocomplete_list.on_cancel = self._cancel_autocomplete
            self._tui.request_render()
    ...
    def render(self, width: int) -> list[str]:
        ...
        visible_lines = layout_lines[
            self._scroll_offset : self._scroll_offset + max_visible_lines
        ]
        result: list[str] = []
        left_pad = " " * padding_x
        right_pad = " " * padding_x
    ...
    def handle_input(self, data: str) -> None:
        ...
        if kb.matches(data, "deleteWordForward"):
            ...
            return
        if kb.matches(data, "deleteCharBackward") or matches_key(
        ...
        if kb.matches(data, "submit"):
            if self.disable_submit:
                return
            current_line = (
                self._state.lines[self._state.cursor_line] if self._state.lines else ""
            )
            if (
        ...
        if kb.matches(data, "cursorUp"):
            if (
                ...
                and self._is_on_first_visual_line()
            ):
    ...
    def _layout_text(self, content_width: int) -> list[_LayoutLine]:
        ...
        if not self._state.lines or (
            len(self._state.lines) == 1 and self._state.lines[0] == ""
        ):
                ...
                for ci, chunk in enumerate(chunks):
                    ...
                    if is_current:
                        ...
                        if is_last:
                            ...
                            has_cursor = (
                                chunk.start_index <= cursor_pos < chunk.end_index
                            )
                            adj_pos = min(
                                cursor_pos - chunk.start_index, len(chunk.text)
                            )
                        if has_cursor:
        ...
        for i, line in enumerate(self._state.lines):
            if not line:
                ...
                for chunk in chunks:
                    visual_lines.append(
                        _VisualLine(
                            i, chunk.start_index, chunk.end_index - chunk.start_index
                        )
                    )
        return visual_lines
    ...
    def _find_current_visual_line(self, visual_lines: list[_VisualLine]) -> int:
        for i, vl in enumerate(visual_lines):
            if vl.logical_line == self._state.cursor_line:
                ...
                if col_in_seg >= 0 and (
                    col_in_seg < vl.length or (is_last and col_in_seg <= vl.length)
                ):
                    return i
    ...
    def _move_to_visual_line(
        ...
        is_last_src = (
            current_vi == len(visual_lines) - 1
            or visual_lines[current_vi + 1].logical_line != current_vl.logical_line
        )
        src_max = current_vl.length if is_last_src else max(0, current_vl.length - 1)

        is_last_tgt = (
            target_vi == len(visual_lines) - 1
            or visual_lines[target_vi + 1].logical_line != target_vl.logical_line
        )
        ...
        logical_line = (
            self._state.lines[target_vl.logical_line] if self._state.lines else ""
        )
    ...
    def _page_scroll(self, direction: int) -> None:
        ...
        current_vi = self._find_current_visual_line(visual_lines)
        target_vi = max(
            0, min(len(visual_lines) - 1, current_vi + direction * page_size)
        )
        self._move_to_visual_line(visual_lines, current_vi, target_vi)
    ...
    def _insert_character(self, char: str, skip_undo_coalescing: bool = False) -> None:
        ...
        if not self._autocomplete_state:
            if char == "/" and self._is_at_start_of_message():
                ...
                text_before = current_line[: self._state.cursor_col]
                if self._is_in_slash_command_context(text_before) or re.search(
                    r"(?:^|[\s])@[^\s]*$", text_before
                ):
                    self._try_trigger_autocomplete()
        else:
    ...
    def _handle_paste(self, pasted_text: str) -> None:
        ...
        if re.match(r"^[/~.]", filtered):
            ...
            char_before = (
                current_line[self._state.cursor_col - 1]
                if self._state.cursor_col > 0
                else ""
            )
    ...
    def _handle_backspace(self) -> None:
        ...
        if self._state.cursor_col > 0:
            ...
            self._state.lines[self._state.cursor_line] = (
                line[: self._state.cursor_col - length] + line[self._state.cursor_col :]
            )
    ...
    def _handle_forward_delete(self) -> None:
        ...
        if self._state.cursor_col < len(current_line):
            ...
            self._state.lines[self._state.cursor_line] = (
                current_line[: self._state.cursor_col]
                + current_line[self._state.cursor_col + length :]
            )
        elif self._state.cursor_line < len(self._state.lines) - 1:
            self._push_undo()
    ...
    def _delete_to_start_of_line(self) -> None:
        ...
        if self._state.cursor_col > 0:
            ...
            self._kill_ring.push(
                deleted, prepend=True, accumulate=(self._last_action == "kill")
            )
            self._last_action = "kill"
            self._state.lines[self._state.cursor_line] = current_line[
    ...
    def _delete_to_end_of_line(self) -> None:
        ...
        if self._state.cursor_col < len(current_line):
            ...
            deleted = current_line[self._state.cursor_col :]
            self._kill_ring.push(
                deleted, prepend=False, accumulate=(self._last_action == "kill")
            )
            self._last_action = "kill"
            self._state.lines[self._state.cursor_line] = current_line[
                : self._state.cursor_col
    ...
    def _insert_yanked_text(self, text: str) -> None:
        ...
        if len(lines_to_insert) == 1:
            ...
            after = current_line[self._state.cursor_col :]
            self._state.lines[self._state.cursor_line] = before + (
                lines_to_insert[0] or ""
            )
            for i in range(1, len(lines_to_insert) - 1):
                self._state.lines.insert(
                    self._state.cursor_line + i, lines_to_insert[i]
                )
    ...
    def _delete_yanked_text(self) -> None:
        ...
        if len(yank_lines) == 1:
            ...
            after_cursor = self._state.lines[self._state.cursor_line][
                self._state.cursor_col :
            ]
    ...
    def _jump_to_char(self, char: str, direction: str) -> None:
        ...
        for line_idx in range(self._state.cursor_line, end, step):
            ...
            if is_forward:
                ...
                idx = (
                    line.rfind(char, 0, search_from + 1)
                    if search_from is not None
                    else line.rfind(char)
                )
    ...
    def _try_trigger_autocomplete(self, explicit_tab: bool = False) -> None:
        ...
        if suggestions and suggestions.items:
            ...
            self._autocomplete_list = SelectList(
                suggestions.items,  # type: ignore[arg-type]
    ...
    def _force_file_autocomplete(self, explicit_tab: bool = False) -> None:
        ...
        if suggestions and suggestions.items:
            if explicit_tab and len(suggestions.items) == 1:
                ...
                self._push_undo()
                self._last_action = None
                result = self._autocomplete_provider.apply_completion(  # type: ignore[arg-type]
                    self._state.lines,
