...
_ARROW_CODEPOINTS_MAP: dict[str, int] = {
    "A": _CP_UP,
    "B": _CP_DOWN,
    "C": _CP_RIGHT,
    "D": _CP_LEFT,
...
def _parse_kitty(data: str) -> _ParsedKitty | None:
    ...
    if m:
        ...
        cp = _FUNC_CODEPOINTS.get(key_num)  # type: ignore[assignment]
        if cp is not None:
            return _ParsedKitty(cp, mod_val - 1, et)
...
def _matches_modify_other_keys(
    data: str, expected_keycode: int, expected_mod: int
) -> bool:
...
def matches_key(data: str, key_id: KeyId) -> bool:
    ...
    if key in ("enter", "return"):
        if shift and not ctrl and not alt:
            if _matches_kitty(data, _CP_ENTER, _MOD_SHIFT) or _matches_kitty(
                data, _CP_KP_ENTER, _MOD_SHIFT
            ):
                return True
        ...
        if alt and not ctrl and not shift:
            if _matches_kitty(data, _CP_ENTER, _MOD_ALT) or _matches_kitty(
                data, _CP_KP_ENTER, _MOD_ALT
            ):
                return True
        ...
        if modifier == 0:
            return (
                ...
                or data == "\x1bOM"
                or _matches_kitty(data, _CP_ENTER, 0)
                or _matches_kitty(data, _CP_KP_ENTER, 0)
            )
        return _matches_kitty(data, _CP_ENTER, modifier) or _matches_kitty(
            data, _CP_KP_ENTER, modifier
        )
    ...
    if key == "insert":
        if modifier == 0:
            return data in _LEGACY_KEY_SEQS["insert"] or _matches_kitty(
                data, _CP_INSERT, 0
            )
    ...
    if key == "delete":
        if modifier == 0:
            return data in _LEGACY_KEY_SEQS["delete"] or _matches_kitty(
                data, _CP_DELETE, 0
            )
    ...
    if key == "pageup":
        if modifier == 0:
            return data in _LEGACY_KEY_SEQS["pageUp"] or _matches_kitty(
                data, _CP_PAGE_UP, 0
            )
    ...
    if key == "pagedown":
        if modifier == 0:
            return data in _LEGACY_KEY_SEQS["pageDown"] or _matches_kitty(
                data, _CP_PAGE_DOWN, 0
            )
    ...
    if key == "left":
        if alt and not ctrl and not shift:
            return (
                ...
                or data == "\x1bb"
                or _matches_kitty(data, _CP_LEFT, _MOD_ALT)
            )
        if ctrl and not alt and not shift:
            return (
                data == "\x1b[1;5D"
                or _matches_legacy_modifier(data, "left", _MOD_CTRL)
                or _matches_kitty(data, _CP_LEFT, _MOD_CTRL)
            )
        if modifier == 0:
    ...
    if key == "right":
        if alt and not ctrl and not shift:
            return (
                ...
                or data == "\x1bf"
                or _matches_kitty(data, _CP_RIGHT, _MOD_ALT)
            )
        if ctrl and not alt and not shift:
            return (
                data == "\x1b[1;5C"
                or _matches_legacy_modifier(data, "right", _MOD_CTRL)
                or _matches_kitty(data, _CP_RIGHT, _MOD_CTRL)
            )
        if modifier == 0:
            return data in _LEGACY_KEY_SEQS["right"] or _matches_kitty(
                data, _CP_RIGHT, 0
            )
...
def parse_key(data: str) -> str | None:
    ...
    if kitty:
        ...
        is_known_sym = chr(cp) in SYMBOL_KEYS if 0 <= cp <= 0xFFFF else False
        effective_cp = (
            cp if (is_latin or is_known_sym) else (kitty.base_layout_key or cp)
        )
        ...
        if effective_cp == _CP_ESCAPE:
            ...
            97 <= effective_cp <= 122
            or 48 <= effective_cp <= 57
            or 0 <= effective_cp <= 0xFFFF
            and chr(effective_cp) in SYMBOL_KEYS
        ):
