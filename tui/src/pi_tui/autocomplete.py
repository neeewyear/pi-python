...
class CombinedAutocompleteProvider:
    ...
    def get_suggestions(
        ...
        if at_prefix is not None:
            ...
            suggestions = self._get_fuzzy_file_suggestions(
                raw_prefix, is_quoted_prefix=is_quoted
            )
            if not suggestions:
                return None
        ...
        if text_before.startswith("/"):
            ...
            if space_idx == -1:
                ...
                for cmd in self.commands:
                    ...
                    cmd_items.append(
                        AutocompleteItem(value=name, label=label, description=desc)
                    )
                ...
                if (
                    command is None
                    or not isinstance(command, SlashCommand)
                    or not command.get_argument_completions
                ):
                    return None
    ...
    def apply_completion(
        ...
        if prefix.startswith("@"):
            ...
            cursor_offset = (
                len(item.value) - 1
                if (is_directory and has_trailing)
                else len(item.value)
            )
            return ApplyResult(
                lines=new_lines,
        ...
        has_trailing = item.value.endswith('"')
        cursor_offset = (
            len(item.value) - 1 if (is_directory and has_trailing) else len(item.value)
        )
        return ApplyResult(
            lines=new_lines,
            cursor_line=cursor_line,
    ...
    def _get_file_suggestions(self, prefix: str) -> list[AutocompleteItem]:
        try:
            ...
            expanded = (
                self._expand_home(raw_prefix)
                if raw_prefix.startswith("~")
                else raw_prefix
            )

            is_root = raw_prefix in ("", "./", "../", "~", "~/", "/") or (
            ...
            if is_root:
                if raw_prefix.startswith("~") or expanded.startswith("/"):
                    ...
                    search_dir = (
                        os.path.join(self.base_path, expanded)
                        if expanded
                        else self.base_path
                    )
                search_prefix = ""
                ...
                if raw_prefix.startswith("~") or expanded.startswith("/"):
                    ...
                    search_dir = (
                        os.path.join(self.base_path, d) if d else self.base_path
                    )
                search_prefix = f
            ...
            for entry in entries:
                ...
                if display_prefix.endswith("/"):
                    ...
                    if display_prefix.startswith("~/"):
                        ...
                        rel_path = (
                            f"~/{name}" if d2 == "." else f"~/{os.path.join(d2, name)}"
                        )
                ...
                value = _build_completion_value(
                    path_val, is_directory, is_at_prefix, is_quoted_prefix
                )
                suggestions.append(
                    AutocompleteItem(
                        value=value, label=name + ("/" if is_directory else "")
                    )
                )
        ...
        try:
            ...
            if slash_idx != -1:
                ...
                if not os.path.isdir(base_dir):
                    display_base = None  # type: ignore[assignment]
            ...
            for (entry_path, is_dir), _ in top:
                ...
                value = _build_completion_value(
                    completion_path, is_dir, True, is_quoted_prefix
                )
                suggestions.append(
                    AutocompleteItem(
                        value=value,
